...
/**
 * Lays out the policy text with the named parties from configuration: the responsible party is
 * the Impressum's operator, and the hoster differs per instance. German, like the Impressum.
 */
  ...
  <div class="flex-1 overflow-auto px-gutter py-5 pb-8 md:px-10">
    <div class="reading-column prose-legal">
      ...
      <p>
        ...
        auf Informationen im Endgerät des Nutzers (z. B. Device-Fingerprinting) im Sinne des TDDDG
      ...
      <address>
        <span class="block">{{ HOSTER.name }}</span>
        <span v-if="HOSTER_ADDRESS?.street" class="block">{{ HOSTER_ADDRESS.street }}</span>
        <span v-if="HOSTER_ADDRESS" class="block">{{ HOSTER_ADDRESS.town }}</span>
      </address>
      ...
      <address>
        <span class="block">{{ WEBSITE_OPERATOR.name }}</span>
        <span v-if="WEBSITE_OPERATOR_ADDRESS?.street" class="block">{{
          WEBSITE_OPERATOR_ADDRESS.street
        }}</span>
        <span v-if="WEBSITE_OPERATOR_ADDRESS" class="block">{{
          WEBSITE_OPERATOR_ADDRESS.town
        }}</span>
      </address>
